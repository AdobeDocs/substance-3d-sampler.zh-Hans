...
breadcrumb-title: ''
description: 查看Substance 3D Sampler版本3.2的发行说明，了解素材数字化工作流程、新滤镜和自定义元数据。
...
**Substance 3D Sampler 3.2**&#x200B;引入了一个端到端的素材数字化工作流程，可捕获和处理素材物理尺寸、布料编织和通道切换等新滤镜，以及创建自定义元数据的功能。
...
此版本中引入了新的素材扫描工作流程，用于捕获和处理素材物理尺寸。

匹配数字环境中采样/图像的实际[物理尺寸](../../features-and-workflows/end-to-end-physical-size-workflow.md)，以便在任何软件中创建物理上准确的材质。
...
将自定义元数据添加到您的素材。 所有自定义元数据都将包含在素材文件(SBSAR)中，以确保跨应用程序共享数字素材的工作流程更加高效。

![](../../assets/custommetadata.png){width="264px"}

### 通道交换机

现在，您可以使用“通道切换”来切换素材输出映射的通道。
...
* [物理尺寸]添加物理尺寸诊断工具
* [物理尺寸]允许设置物理尺寸的z值
* [物理尺寸]用于在2D视图中设置缩放级别的下拉构件
* [物理尺寸]缩放级别下拉列表中新增了“物理比例显示”选项
* [物理尺寸]缩放级别下拉列表中新增了“适合物理尺寸”选项
* [物理尺寸]在2D视图中显示物理尺寸
* [物理尺寸]在3D视区中显示物理尺寸
* [物理尺寸]在图像导入对话框中，如果存在导入的物理尺寸映射，则显示Height深度
...
* [应用程序]实时引擎2021不再可用
* [应用程序]撤消/重做现在支持平铺(U，V)和Height缩放滑块更改
* [渲染]保存所创作的资源时生成磁盘缓存
...
* [内容]用于木质光泽的新图标
* [Content]所有滤镜现在都与Adobe标准材质(ASM)通道兼容
* [内容]滤镜现在可以有“环境”变体

**已修复：**

* [2D视图]通道在删除时保留在列表中
* [应用程序]无法复制从操作系统文件资源管理器加载的资源
* [应用程序]退出时崩溃
* [应用程序]在“资源”面板中单击“入门资源”时，有时会崩溃
* [应用程序]删除材料时崩溃
...
* [图层]微调位于图像到材质(AI)下方的图层时，有时会不计算该图层
* [图层]图像到材质(AI)有时会在不需要时重新计算
* [图层]在磁盘上更新自定义滤镜时，建议不要进行任何更新
* [图层]正常通道有时具有错误的像素格式
* [图层]某些图层即使不可见也仍会计算
* [图层]切换图层可见性时，2D视图工具可能会损坏
* [图层]使用“图像到材质”(AI)时UI冻结
* [图层]切换变换滤镜图层的可见性会破坏2D视图工具，并且可能会导致崩溃
* [图层]从图层栈栈中删除图层时重新计算的次数过多
* [图层]当复合滤镜包含异常或自定义输入/输出时，Sampler不对其进行计算
* [性能]“资源”面板打开缓慢
* [性能]避免对图层栈栈进行一些不必要的重新计算
* [性能]加载项目资源花费的时间过多
* [Performance]不能使用磁盘上的渲染缓存
* [性能]在图层之间切换缓慢
* [性能]调整材质或滤镜时速度缓慢
* [Project]退出时保存项目可能会导致崩溃
* [渲染]删除图像可能会删除所有输出
* [渲染]微调时，视区中显示的渲染时间错误
