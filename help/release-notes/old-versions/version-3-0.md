...
**Substance 3D Sampler 3.0.0**&#x200B;已连接到Adobe Creative Cloud，因此是Substance Alchemist的新名称。 它提供了完整的UI重工、对创建环境光线的支持、完全重工和新的滤镜、发送到功能和ASM着色器支持。
...
Sampler现在让您不仅仅制作素材。 环境光是一种新型资源，具有其[自己的滤镜集](../../filters/hdri-tools/hdri-tools.md)。 从[包围的360张照片](../../filters/hdri-tools/hdr-merge.md)开始，[从头](../../filters/hdri-tools/shape-light.md)构建环境光[编辑现有HDR文件](../../filters/hdri-tools/nadir-patch.md)。
...
Sampler现在可以[使用Substance 3D Painter和Stager轻松共享素材和光照环境](../../interface/panels/share-panel.md)，只需单击一下即可。
...
* [Project]直接从操作系统资源管理器打开.ssa项目文件
* [项目]从操作系统资源管理器打开.sbsar文件将启动Sampler，同时显示一个可以使用此.sbsar文件的新项目
* [项目]从操作系统资源管理器中打开.alch（旧版Substance Alchemist文件）
* [项目面板]包含在一个项目中创建的所有资源的新面板
* [项目面板]使用+图标创建资源（素材或环境光照）
* [项目面板]右键单击资源可打开上下文菜单
...
* [环境光]在环境光创建模板中，选择“环境导入”，以将图像在3D视图中分配给环境
* [环境光]在环境光创建模板中，选择HDR合并，以便从多个具有不同曝光度的360度图像中创建环境光
* [环境光]在环境光创建模板中，选择“用作位图”以在创建环境光之前编辑图像
* [环境光]在“图像导入”图层中指定环境使用情况，以便直接在3D视图中将该图像分配给环境
* [环境光]在环境通道的2D视图中，有一个自动颜色校正使渲染显示与3D视图中的显示相同
* [环境光]用于创建环境光的新专用内容
* [资源面板]“资源”和“过滤器”面板合并到一个新的“资源”面板中
* [资源面板]“资源”面板现在支持以下资源类型：素材、滤镜和图像
...
* [图层面板]图层现在具有缩略图预览（素材缩略图、滤镜图标或图像预览）
...
* [导出]将材质和环境光照发送到Adobe Substance 3D Painter
* [导出]将环境光发送到Adobe Substance 3D Stager
* [渲染]现在支持新的材质属性并以3D形式渲染
* [渲染]添加光泽支持（光泽颜色、光泽不透明度和光泽粗糙度）
* [渲染]添加涂层支持（涂层颜色、涂层粗糙度、涂层标准、涂层Specular level和涂层IOR）
* [渲染]添加各向异性支持（各向异性级别和各向异性角度）
* [渲染]添加Specular edge color支持
* [渲染]在“通道设置”面板中激活这些新属性
* [渲染]在Beta版中引入了新的实时引擎(2021)渲染器
* [渲染]在“查看器设置”面板中的两个渲染器版本之间切换
* [渲染]实时引擎(2021)渲染器支持半透明、吸收和散射材料属性
* [渲染]实时引擎(2021)渲染器引入了一种从环境光计算阴影的新方法
* [渲染]实时引擎(2021)渲染器实时计算环境光的辐照度
* [着色器设置面板]新增着色器设置面板，可微调特定素材着色器参数
* [着色器设置面板]新参数（“正常比例”、“Height比例”、“Height度”、“发射强度”、“IOR”、“皮毛正常强度”和“皮毛IOR”）
* [着色器设置面板]实时引擎2021的特定参数（子表面散射、散射距离、红移和瑞利散射）
* [着色器设置面板]设置值按资源存储
* [查看器设置面板]添加了默认环境光照的预览
* [查看器设置面板]添加了默认网格的预览
* [查看器设置面板]新的环境不透明度参数
* [查看器设置面板]新环境模糊参数（特定于实时引擎2021渲染器）
* [本地化]德语和法语的新翻译
* [内容]新的默认入门材料
* [内容]新的默认环境光照
* [内容]所有过滤器均已更新、清除和优化
...
* [内容]颜色替换 — 在新的受支持通道中进行替换（光泽、涂层、各向异性...）
* [内容]颜色变化 — 手动模式可精确选择要更改的颜色
* [内容]拼贴 — 用于可视化接缝剪切的选项
* [内容]拼贴 — 用于涂抹已剪切的接缝以实现完美拼贴的选项
* [内容]匹配 — 用于添加材质以匹配其颜色和粗糙度的选项
...
* [实时引擎2021]更改布局，使应用程序崩溃
* [实时引擎2021]计算繁重，应用程序崩溃
* [面板] MacOS — 未停靠的面板位于所有应用程序之前
* [构件]变换和位置构件可能会消失。 隐藏和取消隐藏图层以使其显示。
* [导出]环境光的SBSAR导出会失去32位深度精度
