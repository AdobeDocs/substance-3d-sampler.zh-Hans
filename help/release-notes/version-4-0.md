...
借助&#x200B;**Substance 3D Sampler 4.0**，您可以使用真实图像通过自动主体蒙版、纹理映射和几何抽取创建3D对象。 此版本引入了一些UX改进，作为Python API中的新可能性。
...
我们集成了摄影测量功能。 摄影测量是从图像中进行测量的技术过程。 这就是Sampler根据一系列照片创建3D网格的方法。

只需从一系列捕捉对象可见表面的照片开始 — 智能手机或DLSR相机的效果非常好。
...
重建3D对象后，使用自动抽取、展开和烘焙来优化结果。

后期处理可帮助您根据需要以及要如何使用它来调整和优化网格和纹理。

重建结果可以生成具有数百万个多边形和高达16K纹理的网格。 这通常不会针对渲染、实时或AR体验进行优化。
...
* 导出环境光： `export_environment_light`
* 导出带或不带纹理的网格： `export_mesh`或`export_3d_object`

我们添加了一个新函数以导入具有特定使用情况的纹理： `import_textures`
...
* [3D 捕捉]从图像创建3D对象
* [3D 捕捉]专用3D 捕捉向导
* [3D 捕捉]在数据集上导入或生成黑白色蒙版
* [3D 捕捉]对齐结果 — 将所有匹配的特征作为点云查看
* [3D 捕捉]对齐结果 — 查看并与与每张对齐照片关联的相机进行交互
* [3D 捕捉]使用定界框构件定义重建区域
* [3D 捕捉]在所有轴上缩放、平移和旋转定界框构件
* [3D 捕捉]定义重构网格的几何精度
* [3D 捕捉]通过创建新版本来优化网格和纹理
* [3D 捕捉]每个版本都会自动缩减到设定的目标脸部号
* [3D 捕捉]后处理步骤自动展开、重新投影纹理，然后烘焙来自高多边形网格的正常Height和AO信息
* [3D 捕捉]将原始结果或版本添加到Sampler项目
* [3D 捕捉]新增一个“网格后期处理”图层，可自动对基础网格图层的详细信息进行抽取、展开、重新投影纹理和烘焙
* [3D 捕捉]新建网格变换图层以缩放、旋转或平移底层网格图层
* [导出]新的“导出”窗口
* [导出]专用设置和UI，具体取决于资源类型(材料、环境光、网格)
* [导出]将网格导出为USD、USDA、USDZ、glTF、glb、obj、fbx、stl
* [导出]导出材料文件时定义Substance类型(SBSAR、SBS)
...
* [脚本]为材料、环境光和3D对象添加了导出功能
* [脚本]向参数添加了标识符、默认值、最小值和最大值、标签以及枚举值
* [脚本]添加了导入\_纹理函数，用于在导入图像时输入自定义用法
...
* [3D 捕捉]如果网格变换中的缩放比例太小，对象上可能会出现小的伪像
