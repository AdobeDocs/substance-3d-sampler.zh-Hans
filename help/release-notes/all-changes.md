---
helpx_url: 'https://helpx.adobe.com/cn/substance-3d-sampler/release-notes/all-changes.html'
breadcrumb-title: ''
...
* &amp;lbrack；Application&amp;rbrack；防止在macOS上退出时崩溃
* &amp;lbrack；Application&amp;rbrack；阻止访问无效资源引用的资源
* &amp;lbrack；Application&amp;rbrack；防止在微调中从VersionedImage访问表面时发生崩溃
* &amp;lbrack；Application&amp;rbrack；防止删除无阶段时崩溃
...
* 启用“缩放”时，&amp;lbrack；滤镜&amp;rbrack；内容识别填充崩溃
...
* &amp;lbrack；Rendering&amp;rbrack；防止删除仍可由Vulkan访问的纹理
* &amp;lbrack；渲染纹理旋转未从0-1转换为0-360
* &amp;lbrack；脚本编写&amp;rbrack；从Python文档中删除不存在的类
* 如果没有选定资源，则&amp;lbrack；Scripting&amp;rbrack； selectedAsset返回None
* &amp;lbrack；Tools&amp;rbrack；重置纹理值现在会停止绘画并清除修补视图
* &amp;lbrack；UI&amp;rbrack；在调整内容时不要关闭“属性”面板中的部分
* &amp;lbrack；UI&amp;rbrack；悬停时显示不可见的公开颜色扭曲标签
...
* &amp;lbrack；UI&amp;rbrack；就环境光编辑的弃用发出警告

**已修复：**

* 使用Substance过滤器时，选择“以物理比例显示”可能无法使用&amp;lbrack；2D视图&amp;rbrack；
* &amp;lbrack；3D 捕捉&amp;rbrack； Svg文件在文件选择器中列出，但不受支持
* 着色器设置中的&amp;lbrack；3D视图&amp;rbrack；发射强度参数不起作用
* &amp;lbrack；3D视图&amp;rbrack；在创建新资源时，有时网格位置不正确
* &amp;lbrack；3D视图&amp;rbrack；切换到路径跟踪渲染会在不受支持的硬件上崩溃
* 关闭手动测量弹出窗口而不设置大小时，&amp;lbrack；应用程序挂起(&amp;R)
* Lbrack；应用程序崩溃(&amp;R)
* 显示桌面时，应用程序&amp;rbrack；在Windows上冻结（Windows键+ D键盘快捷键）(&amp;L)
* &amp;lbrack；Application&amp;rbrack；切换语言时可能崩溃
* &amp;lbrack；Captis&amp;rbrack；预览数据无效时崩溃
...
* &amp;lbrack；UI&amp;rbrack；纹理tilling V参数在查看器设置和3D视图中部分隐藏
...
* &amp;lbrack；Captis&amp;rbrack；允许为素材指定与现有素材相同的名称
...
* &amp;lbrack；3D视图&amp;rbrack；3D视图由于着色器缓存损坏而无法工作
* 场景为Z-up时，&amp;lbrack；3D视图&amp;rbrack；地平面和网格垂直
* &amp;lbrack；3D视图&amp;rbrack；网格有时消失
* &amp;lbrack；Application&amp;rbrack；启动时关闭登录窗口而不登录有时会使应用程序崩溃
* &amp;lbrack；Application&amp;rbrack；访问插件配置文件被拒绝时崩溃
* 保存项目时未选择当前材料(&amp;L)
* &amp;lbrack；Application&amp;rbrack；重置为默认布局会将分辨率设置为64x64
* 渲染图层栈栈时，&amp;lbrack；Application&amp;rbrack； Sampler有时会崩溃
* &amp;lbrack；Export&amp;rbrack；导出分辨率有时重置为64x64
* &amp;lbrack；Export&amp;rbrack；有时无法导出.sbs/.sbsar文件
* 当素材为空时，&amp;lbrack；图层&amp;rbrack；“添加基础材质”按钮不起作用
* 复制材质时，图层&amp;brack；纹理拼贴发生更改(&amp;L)
...
* &amp;lbrack；UI&amp;rbrack；滑块在需要时不会对齐到整数值
...
* &amp;lbrack；Application&amp;rbrack；导入的图像不出现在视口或图层栈栈中
* &amp;lbrack；Captis&amp;rbrack；即使在重新启动Sampler后，Captis IP地址字段仍为空
* &amp;lbrack；Captis&amp;rbrack；实时摄像机预览仅在应用程序语言设置为英语时有效
* &amp;lbrack；Export&amp;rbrack；在导出过程中崩溃&amp;lbrack；Layers&amp;rbrack；绘画有时在以前保存的项目中不起作用
* 仅更新一个通道时，&amp;lbrack；Layers&amp;rbrack； Sampler有时会更新所有纹理
* &amp;lbrack；Layers&amp;rbrack；升级到5.0.x后无法在图层栈栈中使用素材混合
...
* &amp;lbrack；UI&amp;rbrack；崩溃报告窗口UI样式
* &amp;lbrack；UI&amp;rbrack；复制到剪贴板时，显示Toast说明操作已完成(&amp;R)

**已修复：**

* 球面投影关闭时，&amp;lbrack；2D视图&amp;rbrack；曝光度滑块不起作用
* &amp;lbrack；2D视图&amp;rbrack；在纹理外部绘画将创建已停用的描边
* &amp;lbrack；2D视图&amp;rbrack； “曝光度”按钮没有工具提示。
* &amp;lbrack；2D视图&amp;rbrack；缩放非正方形图像的侧边时不再跟随鼠标
* &amp;lbrack；3D 捕捉&amp;rbrack；3D 捕捉在Windows 11 24H2上不起作用
* &amp;lbrack；3D 捕捉&amp;rbrack；在网格重建步骤中退出Sampler时崩溃
* &amp;lbrack；3D视图&amp;rbrack；计算时间有时显示为0毫秒
* &amp;lbrack；3D视图&amp;rbrack；将投影从正交更改为透视时，视区变为灰色
* 检查GPU功能时&amp;lbrack；Application&amp;rbrack；启动时崩溃
* &amp;lbrack；Application&amp;rbrack；在安装过程中崩溃
* 右键单击元数据字段后&amp;lbrack；Application&amp;rbrack；退出时崩溃
* 从操作系统文件资源管理器打开SBSAR时，&amp;lbrack；Application&amp;rbrack；环境光丢失
...
* &amp;lbrack；Layers&amp;rbrack；在使用“放大”滤镜时崩溃
* &amp;lbrack；Layers&amp;rbrack；将包含旧图像的项目更新为素材可使素材变为全黑色
* &amp;lbrack；在创建资源后立即调整图层栈栈会中断渲染(&amp;R)
...
* &amp;lbrack；载入&amp;rbrack； &amp;lbrack；内容快速操作是预定义的工作流，使用最常用的图层填充图层栈栈
* &amp;lbrack；入门&amp;rbrack；可通过新的快速入门菜单、快速操作或自定义项目创建新项目
* &amp;lbrack；入门&amp;rbrack；可通过专用按钮直接从主页创建空项目
* &amp;lbrack；3D View&amp;rbrack；新的高级栅格化器和路径跟踪器在Substance生态系统中带来了新的渲染功能（如涂层、光泽、半透明、次表面散射等特性）和视觉一致性
* 现在可以直接在3D视图中访问&amp;lbrack；3D视图&amp;rbrack；查看器设置
* &amp;lbrack；3D视图&amp;rbrack；可将渲染快照保存在剪贴板或文件中
* &amp;lbrack；3D视图&amp;rbrack；显示网格以可视化场景原点
* &amp;lbrack；3D视图&amp;rbrack；启用地平面以捕捉阴影和反射
* &amp;lbrack；3D视图&amp;rbrack；控制地平面反射和不透明的程度
* &amp;lbrack；3D 捕捉&amp;rbrack；定位地面网格
* &amp;lbrack；Application&amp;rbrack；检查应用程序启动时的硬件兼容性
* &amp;lbrack；Application&amp;rbrack；崩溃报告窗口现在会在崩溃发生后立即打开
* &amp;lbrack；Content&amp;rbrack；打开示例项目以轻松开始
* &amp;lbrack；导出&amp;rbrack；以USD文件格式导出Adobe标准素材着色器
* &amp;lbrack；生成式AI&amp;rbrack；在“图像到纹理”工作流程中使用图像作为输入时，请勾选“不推断”标记
...
* &amp;lbrack；脚本编写&amp;rbrack；在一个脚本中多次更改图层栈栈结构可能会失败
...
* &amp;lbrack；Content&amp;rbrack；新的纹理生成器类别 — 渐变
* &amp;lbrack；Content&amp;rbrack；HDRI 工具 — 新的环境旋转过滤器

**已修复：**

* 公开.sbsar输入值时不起作用(&amp;L)
* &amp;lbrack；图层&amp;rbrack；基色在灰度图像时将变为红色
* 颜色通道中使用的灰度图像具有错误的色彩空间(&amp;L)
* &amp;lbrack；脚本编写&amp;rbrack；使用导出预设有时无法导出预期的通道
* &amp;lbrack；Content&amp;rbrack；Dirt — 将图像上的Dirt滤镜应用于素材可生成黑色法线
* &amp;lbrack；Content&amp;rbrack；浮雕 — 浮雕滤镜中图案的缩放在0和1之间不是线性的
...
* 使用“图像到纹理”时，有时会出现&amp;lbrack；生成式AI&amp;rbrack；网络错误
...
* &amp;lbrack；Application&amp;rbrack；打开损坏的项目文件时崩溃
* &amp;lbrack；Application&amp;rbrack；在项目包含损坏的资源时崩溃
* 在Windows上拔下监视器时&amp;lbrack；Application&amp;rbrack；崩溃
...
* &amp;lbrack；Filters&amp;rbrack； Upscale可能会根据其输入图层崩溃
* &amp;lbrack；生成式AI&amp;rbrack；在收到来自服务的意外结果时可能会崩溃
...
* &amp;lbrack；3D 捕捉&amp;rbrack；在批量生成蒙版时无法访问图像文件时崩溃
* &amp;lbrack；Export&amp;rbrack；使用“裁剪”或相对于输入策略层导出素材时会产生无效结果
...
* 资源包；资源&amp;资源包；新资源类型：纹理生成器(&amp;L)
* &amp;brack；资源&amp;brack；入门资源中包含的新材料
* &amp;lbrack；资源&amp;rbrack；在“属性”面板中为图像参数新建资源选择器
* &amp;lbrack；资源&amp;rbrack；将纹理生成器从“资源”面板拖放到“属性”面板中的图像选择器中
* &amp;lbrack；资源&amp;rbrack；从操作系统文件资源管理器中拖放纹理生成器
* &amp;lbrack；Assets&amp;rbrack；滤镜可以通过图像输入上的用户标记建议适合生成器
* &amp;lbrack；资源&amp;rbrack；纹理生成器可以定义哪个过滤器应通过用户标记提供建议
* &amp;lbrack；Content&amp;rbrack；新透视裁剪滤镜
* &amp;lbrack；Content&amp;rbrack；新风格化滤镜
* 填充滤镜上的混合模式(&amp;L)&amp;R)
* &amp;lbrack；Content&amp;rbrack；更新的刺绣滤镜
* &amp;lbrack；Content&amp;rbrack；更新了“颜料换行”滤镜
* &amp;lbrack；Content&amp;rbrack；更新了所有滤镜以支持纹理生成器
* &amp;lbrack；Layers&amp;rbrack；将纹理生成器输出通道添加到图层栈叠时，能够选择它
* &amp;lbrack；Layers&amp;rbrack；能够轻松列出和应用纹理生成器上的预设
* &amp;lbrack；图层&amp;rbrack；在图像选择器中显示纹理生成器预览
* 可以显示和导出&amp;lbrack；图层&amp;rbrack；纹理生成器参数
* &amp;lbrack；Layers&amp;rbrack；在使用纹理导入创建模板导入单个图像时指定基色使用情况
* 尝试在“属性”面板的图像拾色器中拖放不兼容文件时&amp;lbrack；图层&amp;rbrack；反馈
* &amp;lbrack；图层&amp;rbrack；通过导入图像的Alpha通道生成不透明度通道
* 更改其类别时，&amp;lbrack；图层&amp;rbrack；图像到材质(AI)的计算速度更快
...
* 初始化图形API时&amp;lbrack；Application&amp;rbrack；在启动时崩溃
* 如果项目名称包含#字符，&amp;lbrack；Application&amp;rbrack；崩溃
* &amp;lbrack；Application&amp;rbrack；打开旧项目时可能崩溃
...
* &amp;lbrack；Export&amp;rbrack；导出的灰度图像.sbs/.sbsar文件的色彩空间有误
...
* &amp;lbrack；Engine&amp;rbrack；将Substance 引擎更新到版本9.0.3
* &amp;lbrack；Layers&amp;rbrack；内容识别填充：上游更新、各种用例修复和Linux支持

**已修复：**

* &amp;lbrack；3D 捕捉&amp;rbrack；在对齐后单击“返回”再单击“下一步”不会更新点云
* &amp;lbrack；3D 捕捉&amp;rbrack；网格在添加到项目后显示有孔
* &amp;lbrack；Application&amp;rbrack；在3D 捕捉后退出全屏模式时崩溃
...
* &amp;lbrack；无法转换3D网格时，Application&amp;rbrack；潜在崩溃
* &amp;lbrack；Application&amp;rbrack；在Sampler运行时打开.sbsar时发生静默崩溃
* &amp;lbrack；导出自定义用途的.sbs/.sbsar文件时，Export&amp;rbrack；崩溃
* &amp;lbrack；Export&amp;rbrack；导出的正常映射始终DirectX，无论用户如何设置
* &amp;lbrack；Export&amp;rbrack；在macos上将3D对象导出为FBX文件不起作用
* &amp;lbrack；Export&amp;rbrack；将带有刺绣滤镜的图层栈叠导出为.sbs/.sbsar文件时出现不一致
* &amp;lbrack；Export&amp;rbrack；有时无法导出.sbs/.sbsar文件
...
* &amp;lbrack；Content&amp;rbrack；图像到材质 — 改进正常映射中的微细节生成
* &amp;lbrack；Content&amp;rbrack；图像到材质 — 新的愉悦强度参数
* 可以在图像导入图层中添加图像(&amp;L)&amp;R)
* 可以在“图像导入”图层中删除&amp;lbrack；图层&amp;rbrack；图像
* 现在可以删除无效的图层(&amp;L)
* &amp;lbrack；2D视图&amp;rbrack；按Shift+C快捷键可循环切换通道
* &amp;lbrack；3D 捕捉&amp;rbrack；当用户导入的图像数少于20个时显示警告toast
* &amp;lbrack；Application&amp;rbrack；设置默认材质纹理拼贴值的新偏好设置
* &amp;lbrack；入门&amp;rbrack；更新了图像到材质(AI)和升级的教程UI
* &amp;lbrack；脚本&amp;rbrack；3D 捕捉API：将Capture3dState设置为对齐时，DatasetInfo包含更多数据
* &amp;lbrack；脚本编写&amp;rbrack；将select_asset参数新增到create_asset()。 新函数：wait_for_computation()和clear_render_cache()

**已修复：**

* &amp;lbrack；图层&amp;rbrack；裁剪区域很小时崩溃
* 添加或调整“裁剪”滤镜时&amp;lbrack；图层&amp;rbrack；崩溃
* &amp;lbrack；图层&amp;rbrack；使裁剪区域变成方形会导致错误的素材输出分辨率
* 禁用多个图层时，&amp;lbrack；图层和rbrack；输出有时会消失
* &amp;lbrack；图层&amp;rbrack；渲染缓存可能无法通过“图像到材质”(AI)和“放大”滤镜正确失效
* &amp;lbrack；Layers&amp;rbrack；在警告弹出窗口中选择“不再显示此消息”时，无法添加“放大”滤镜
* &amp;lbrack；图层&amp;rbrack；修改后，无法恢复刺绣滤镜中的图像
* &amp;lbrack；Export&amp;rbrack；在更改正常格式时，导出的正常映射分辨率发生变化
* &amp;lbrack；导出&amp;rbrack；导出环境时删除“\_environment”文件名后缀
* &amp;lbrack；Export&amp;rbrack；在图层栈栈中存在变形变换图层时无法导出.sbsar文件
* 分辨率更改时，&amp;lbrack；2D视图&amp;rbrack；“适合屏幕”不起作用
...
* &amp;lbrack；Content&amp;rbrack；极大地改进了“图像到材质”(AI)和Delighter滤镜
* &amp;lbrack；Content&amp;rbrack；新建放大滤镜
* &amp;lbrack；Content&amp;rbrack； “裁剪”滤镜现在具有动态输出分辨率。
* &amp;lbrack；材质创建模板&amp;rbrack；添加文档大小设置。
* &amp;lbrack；材质创建模板&amp;rbrack；新的“添加裁剪”切换按钮。
* &amp;lbrack；材质创建模板&amp;rbrack；新的“放大材质”切换开关
* &amp;lbrack；材质创建模板&amp;rbrack；显示导入的图像大小
* &amp;lbrack；材质创建模板&amp;rbrack；在某些导入的图像无法使用时提供反馈
* &amp;lbrack；材质创建模板&amp;rbrack；图像大小不一致时发出警告
...
* &amp;lbrack；UI&amp;rbrack； 3D视区载入工具提示有焦点问题
...
* &amp;lbrack；Engine&amp;rbrack；将Substance 引擎更新到版本9.0
* &amp;lbrack；互操作性&amp;rbrack；将3D对象发送到Stager和Painter

**已修复：**

* 3D 捕捉渲染器出现故障时&amp;lbrack；3D 捕捉&amp;rbrack；应用程序崩溃
* &amp;lbrack；无法加载图像时3D 捕捉&amp;rbrack；崩溃
* &amp;lbrack；3D 捕捉&amp;rbrack；达到网格重构步骤时崩溃
* 调整定界框大小时崩溃(&amp;L)；3D 捕捉(&amp;R)
...
* 将图像导出到材质（AI驱动）图层到.sbs/.sbsar文件时，&amp;lbrack；Export&amp;rbrack；保留拼贴不起作用
...
* &amp;lbrack；Project&amp;rbrack；保存时可能崩溃
* &amp;lbrack；UI&amp;rbrack；在版本的菜单按钮上添加缺少的间距
* 无法正确显示&amp;lbrack；UI&amp;rbrack；取消按钮
* &amp;lbrack；UI&amp;rbrack；禁用3D 捕捉后处理参数的滑块动画
* &amp;lbrack；UI&amp;rbrack；在外部单击时，“材质创建模板”窗口不会自行关闭
...
* &amp;lbrack；Content&amp;rbrack；色温滤镜现在处理材质
* &amp;lbrack；Content&amp;rbrack；正常使用Height滤镜有一个保留拼贴的新选项
...
* &amp;lbrack；3D 捕捉&amp;rbrack；生成新版本时崩溃
* 尝试在查看器中加载3D对象时&amp;lbrack；3D 捕捉&amp;rbrack；崩溃
* 将路径用于非UTF8字符时&amp;lbrack；3D 捕捉&amp;rbrack；崩溃
* &amp;lbrack；3D 捕捉&amp;rbrack；命中与提示拼写错误
* &amp;lbrack；3D 捕捉&amp;rbrack；网格不再缩放以适合单位立方体
* &amp;lbrack；3D 捕捉&amp;rbrack；防止在渲染时关闭3D 捕捉时发生崩溃
* &amp;lbrack；3D 捕捉&amp;rbrack；移除蒙版会使图像消失
* 同时导入两次资源时，Application&amp;rbrack；崩溃(&amp;L)
* &amp;lbrack；Application&amp;rbrack；在打开项目时备份以前版本的资源（如果从未备份过）
* &amp;lbrack；Application&amp;rbrack；在并非所有映射都已内置时正确缓存已烘焙贴图
* 显示3D对象时，&amp;lbrack；Application&amp;rbrack；全屏崩溃。
* &amp;lbrack；保存项目时复制Application&amp;rbrack；最后一个材质
* &amp;lbrack；Application&amp;rbrack；防止在烘焙步骤中取消网格后处理计算时发生崩溃
* &amp;lbrack；Application&amp;rbrack；重新打开当前项目不会放弃更改
* &amp;lbrack；应用程序&amp;rbrack；停止生成3D对象的缩览图
* &amp;lbrack；2D视图&amp;rbrack；使用画笔工具时崩溃
* &amp;lbrack；Content&amp;rbrack；内容识别填充 — 计算可能会停滞
...
* &amp;lbrack；3D 捕捉&amp;rbrack；在数据集上导入或生成黑白蒙版
* &amp;lbrack；3D 捕捉&amp;rbrack；对齐结果 — 将所有匹配功能作为点云查看
* &amp;lbrack；3D 捕捉&amp;rbrack；对齐结果 — 查看每个对齐的照片的相机并与之交互
* &amp;lbrack；3D 捕捉&amp;rbrack；使用定界框构件定义重建区域
* &amp;lbrack；3D 捕捉&amp;rbrack；缩放、平移和旋转定界框构件的所有轴(&amp;l)
* &amp;lbrack；3D 捕捉&amp;rbrack；定义重构网格的几何精度
* &amp;lbrack；3D 捕捉&amp;rbrack；通过创建新版本来优化网格和纹理
* &amp;lbrack；3D 捕捉&amp;rbrack；每个版本都会自动抽取到设定的目标脸孔数
* &amp;lbrack；3D 捕捉&amp;rbrack；后处理步骤自动展开、重新投影纹理，然后从高多边形网格中烘焙常规Height和AO信息
* &amp;lbrack；3D 捕捉&amp;rbrack；将原始结果或版本添加到Sampler项目
* &amp;lbrack；3D 捕捉&amp;rbrack；新建网格后处理图层以自动对底层网格图层进行缩减、展开、重新投影纹理和烘焙细节
* &amp;lbrack；3D 捕捉&amp;rbrack；新建网格变换图层以缩放、旋转或平移底层网格图层
* &amp;lbrack；Export&amp;rbrack；新建导出窗口
* &amp;lbrack；导出&amp;rbrack；专用设置和UI，具体取决于资源类型（材质、环境光照、网格）
* &amp;lbrack；导出&amp;rbrack；将网格导出为USD、USDA、USDZ、glTF、glb、obj、fbx、stl
* &amp;lbrack；Export&amp;rbrack；在导出Substance文件(SBSAR、SBS)时定义材料类型
* &amp;lbrack；UI&amp;rbrack；在“首选项”弹出窗口中将缓存设置移动到新选项卡
* 现在可以对2D和3D视口进行垂直调整大小、交换和栈叠(&amp;lbrack)
...
* &amp;lbrack；脚本&amp;rbrack；添加了材质、环境光照和3d对象的导出功能
* &amp;lbrack；脚本&amp;rbrack；向参数中添加了标识符、默认值、最小值和最大值、标签以及枚举值
* &amp;lbrack；脚本&amp;rbrack；添加了import_textures函数，用于在导入图像时输入自定义用法

**已修复：**

* 打开最近的项目并在确认对话框中保存时&amp;lbrack；Application&amp;rbrack；崩溃
* Application&amp;rbrack；的“文件”对话框阻止打开.ssa文件(&amp;L)
* &amp;lbrack；Application&amp;rbrack；文件对话框可以在macOS的后台窗口中显示
* &amp;lbrack；Application&amp;rbrack；打开3.2项目时潜在崩溃
* &amp;lbrack；Application&amp;rbrack；选择文件会在显示警告之前关闭“文件”对话框
* &amp;lbrack；暴露的参数&amp;rbrack；导出参数环境光不起作用
* 图层栈栈中的&amp;lbrack；图层&amp;rbrack；“单击此处浏览”链接不再有效
* 有时无法在同一图层中绘制多个图像(&amp;L)
...
* &amp;lbrack；3D 捕捉&amp;rbrack；使用蒙版时，纹理投影可能会损坏
* 如果网格变换中的缩放比例太小，则对象上可能会出现小的伪影(&amp;L)。3D 捕捉(&amp;R)
...
* &amp;lbrack；脚本编写&amp;rbrack；用于启用打印到“日志”面板的新ssa.helpers模块
* &amp;lbrack；Application&amp;rbrack；支持Substance 3D Designer中新增的并排按钮Widget

**已修复：**

* &amp;lbrack；导出&amp;rbrack；引用丢失的图像的.sbsar文件时崩溃
* &amp;lbrack；导出引用损坏的图像文件的资源时崩溃(&amp;R)
...
* &amp;lbrack；公开的参数&amp;rbrack；新建公开参数面板
* &amp;lbrack；公开参数&amp;rbrack；参数悬停时的新建按钮，用于从“属性”面板公开和取消公开参数
* &amp;lbrack；公开参数&amp;rbrack；新增右键单击参数上下文菜单，以从“属性”面板公开和取消公开参数
* 公开参数面板中列出了&amp;lbrack；公开的参数&amp;rbrack；公开的参数
* &amp;lbrack；公开参数&amp;rbrack；色点和色盘添加到多个位置以轻松识别公开参数
* 可以在公开参数面板中编辑&amp;lbrack；公开参数&amp;rbrack；参数标签
* &amp;lbrack；暴露的参数&amp;rbrack；显示不可导出参数的警告
* &amp;lbrack；曝光参数&amp;rbrack；如果将具有曝光混合参数的图层移动到其变为隐藏的位置，则显示警告
* 外露参数&amp;lbrack；外露参数以SBS和SBSAR格式导出
...
* &amp;lbrack；Content&amp;rbrack；图像到材质（AI驱动）现在可在MacOS上使用
* &amp;lbrack；Engine&amp;rbrack；将Substance引擎更新到8.6.0版

**已修复：**

* &amp;lbrack；Application&amp;rbrack；应用程序可能会在缩略图生成过程中退出时崩溃
* 退出时使用“另存为”时，&amp;lbrack；Application&amp;rbrack；应用程序可能会崩溃
...
* 切换资源时&amp;lbrack；Project&amp;rbrack；不必要的计算
* &amp;lbrack；渲染某些图层栈叠在删除图层后无法正确渲染(&amp;R)
...
* &amp;lbrack；Application&amp;rbrack；修复打开项目时潜在的崩溃问题
* &amp;lbrack；Export&amp;rbrack；重新启动Sampler会中断导入的自定义导出预设列表
* &amp;lbrack；Interoperability&amp;rbrack；修复从Designer发送的材料被删除，然后从Designer重新发送时崩溃的问题
* &amp;lbrack；Project&amp;rbrack；无法删除项目中的最后一个资源或环境光(&amp;l)
* &amp;lbrack；Project&amp;rbrack；右键单击环境光会显示“未保存的修改”星号
...
* 创建环境光时&amp;lbrack；2D视图&amp;rbrack；新建球面投影模式
...
* &amp;lbrack；物理尺寸&amp;rbrack；在编辑物理尺寸时保持与非方形纹理的比例
* &amp;lbrack；物理尺寸&amp;rbrack；物理尺寸元数据在物理尺寸更改时立即应用
* &amp;lbrack；UI&amp;rbrack；调整Height缩放最大滑块，以便在启用物理尺寸时影响任何类型的材质
...
* 裁剪和变换工具中的&amp;lbrack；2D视图&amp;rbrack；90度旋转按钮无法按预期工作
* &amp;lbrack；2D视图&amp;rbrack；裁切构件有时会丢失
* &amp;lbrack；Application&amp;rbrack；清除图像参数不会重新连接基础层
* &amp;lbrack；Application&amp;rbrack；保存项目后退出时崩溃
* 将当前素材拖放到“资源”面板的集合中时&amp;lbrack；Application&amp;rbrack；崩溃
* &amp;lbrack；Application&amp;rbrack；在视口中拖放资源可能会崩溃
...
* &amp;lbrack；Content&amp;rbrack；刺绣滤镜：移除金属结构图中的螺纹
* &amp;lbrack；Content&amp;rbrack；地板拼贴滤镜：修复x和y拼贴计数
* &amp;lbrack；Content&amp;rbrack；砖墙滤镜：将法线和Height输出到16位
* &amp;lbrack；Export&amp;rbrack；导出弹出窗口中的默认文件名不是当前材质名称
...
* 用于在2D视图中设置缩放级别的&amp;lbrack；物理尺寸&amp;rbrack；下拉构件
* &amp;lbrack；物理尺寸&amp;rbrack；在缩放级别下拉列表中新增了“以物理比例显示”选项
* &amp;lbrack；物理尺寸&amp;rbrack；缩放级别下拉列表中新增了“适合物理尺寸”选项
* &amp;lbrack；物理尺寸&amp;rbrack；在2D视图中显示物理尺寸
* &amp;lbrack；物理尺寸&amp;rbrack；在3D视口中显示物理尺寸
* &amp;lbrack；物理尺寸&amp;rbrack；在图像导入对话框中，如果存在导入的物理尺寸映射，则显示Height深度
* &amp;lbrack；物理尺寸&amp;rbrack；在资源上下文菜单中显示物理尺寸
* &amp;lbrack；物理尺寸&amp;rbrack；在首选项中设置长度单位
* &amp;lbrack；物理尺寸&amp;rbrack；根据物理比率导出纹理
...
* &amp;lbrack；Content&amp;rbrack；所有滤镜现在都与Adobe标准材质(ASM)通道兼容
...
* &amp;lbrack；Application&amp;rbrack；退出时崩溃
* 在“资源”面板中单击“入门资源”时，&amp;lbrack；Application&amp;rbrack；有时会崩溃
* &amp;lbrack；应用程序在删除材质时崩溃(&amp;R)
* &amp;lbrack；Application&amp;rbrack；环境变量“SUBSTANCE_DISABLE_SPECIFIC_FEATURES”在设置为“0”或“”时仍处于活动状态。
* 保存包含多种素材的项目时应用程序冻结(&amp;L)
* 导入图像可能会导致崩溃(&amp;L)
...
* 调整图层下面的图层时，&amp;lbrack；图层&amp;rbrack；图像到材质(AI)有时会不计算
* &amp;lbrack；Layers&amp;rbrack；图像到材质(AI)有时会在不需要时重新计算
* &amp;lbrack；Layers&amp;rbrack；在磁盘上更新自定义筛选器时，建议不要更新
* Layers&amp;rbrack；正常通道有时具有错误的像素格式(&amp;L)
* &amp;lbrack；Layers&amp;rbrack；某些图层即使不可见也仍会计算
* &amp;lbrack；图层&amp;rbrack；切换图层可见性时，2D视图工具可能会损坏
* &amp;lbrack；Layers&amp;rbrack；使用“图像到材质”(AI)时，UI冻结
* &amp;lbrack；Layers&amp;rbrack；切换变换滤镜图层的可见性会中断2D视图工具，并可能导致崩溃
* 从图层栈栈中删除图层时，重新计算的次数过多(&amp;L)
* &amp;lbrack；Layers&amp;rbrack；当复合滤镜包含异常或自定义输入/输出时，Sampler不对其进行计算
* &amp;lbrack；Performance&amp;rbrack；资源面板打开缓慢
* &amp;lbrack；Performance&amp;rbrack；避免对图层栈栈进行一些不必要的重新计算
* &amp;lbrack；Performance&amp;rbrack；加载项目资源花费的时间过多
* 不能使用磁盘上的渲染缓存(&amp;L)
* &amp;lbrack；Performance&amp;rbrack；在图层之间切换缓慢
* &amp;lbrack；性能微调素材或滤镜速度缓慢
* &amp;lbrack；退出时保存项目可能会导致崩溃(&amp;R)
* &amp;lbrack；渲染&amp;rbrack；删除图像可能会删除所有输出
* &amp;lbrack；Rendering&amp;rbrack；调整时，视口中显示的渲染时间不正确
...
* &amp;lbrack；UI&amp;rbrack；重命名两种材质后，文本字段不会消失
...
* &amp;lbrack；Application&amp;rbrack；在2D视图中，正常格式显示在首选项中设置的正常格式之后
* &amp;lbrack；Application&amp;rbrack；以首选项中设置的常规格式导出常规格式
* &amp;lbrack；Export&amp;rbrack；将常规格式参数添加到SBS和SBSAR文件导出
* &amp;lbrack；导出&amp;rbrack；将着色器设置添加到SBS和SBSAR文件导出
* &amp;lbrack；Export&amp;rbrack；设置导出的SBS图形的默认分辨率
...
* &amp;lbrack；Engine&amp;rbrack；将Substance引擎更新到v8.2.0

**已修复：**

* &amp;lbrack；Application&amp;rbrack；连接的本地文件夹可能会挂起
* &amp;lbrack；Application&amp;rbrack；退出时崩溃
* 启动两个Sampler实例时&amp;lbrack；Application&amp;rbrack；崩溃
...
* &amp;lbrack；复合滤镜&amp;rbrack；使用复合滤镜对图层栈叠重新排序时崩溃
* &amp;lbrack；复合滤镜&amp;rbrack；渲染有时会挂起
* &amp;lbrack；图像导入&amp;rbrack；导入图像会触发多个渲染
* &amp;lbrack；还原/重做时图层&amp;rbrack；崩溃
* 添加基础材质时&amp;lbrack；Layers&amp;rbrack；崩溃
* &amp;lbrack；Layers&amp;rbrack；在将无效图像用作环境光时崩溃
* &amp;lbrack；图层&amp;rbrack；修复插入具有多个图表的过滤器时重复的导入
* &amp;lbrack；Layers&amp;rbrack；重新排序图层并非始终有效
* &amp;lbrack；Project&amp;rbrack；加载不完整的项目文件时崩溃
* &amp;lbrack；Project&amp;rbrack；打开损坏的项目时崩溃
...
* &amp;lbrack；UI&amp;rbrack；创建环境光模板窗口中的新说明文本
* &amp;lbrack；Application&amp;rbrack；在发送到外部应用程序时，以当前分辨率导出资源
* &amp;lbrack；应用程序&amp;rbrack；材质默认分辨率现在为2048\*2048（在macos上为1024\*1024）
* Floor拼贴滤镜中的新图案(&amp;L)
* 颜色替换滤镜中的&amp;lbrack；Content&amp;rbrack；新双色模式

**已修复：**

* 画笔工具中的2D视图&amp;rbrack；第一个描边有时损坏(&amp;L)
* &amp;lbrack；2D视图&amp;rbrack；画笔工具不可见时释放资源
* &amp;lbrack；2D视图&amp;rbrack；在变换Widget中使用右侧的调整大小光标
* 如果用户之前已在2D视图中平移，&amp;lbrack；2D视图&amp;rbrack；构件不会显示
* &amp;lbrack；Application&amp;rbrack；打开具有损坏工作流的项目时崩溃
...
* &amp;lbrack；Layers&amp;rbrack；避免在输入计数不匹配时崩溃
* 插入基础材质图层时&amp;lbrack；图层崩溃(&amp;R)
* &amp;lbrack；图层&amp;rbrack；筛选器输入计数上限为默认值
* &amp;lbrack；图层&amp;rbrack；重做操作将混合类型错误地更改为Height混合
...
* 混合图层可以停止使用(&amp;L)&amp;R；
* &amp;lbrack；Project&amp;rbrack；加载缺少依赖项文件夹的旧项目时崩溃
* &amp;lbrack；Project&amp;rbrack；在保存后使用撤消/重做时崩溃
* &amp;lbrack；Project&amp;rbrack；打开包含环境光的SBSAR文件将创建素材资源
...
* &amp;lbrack；UI&amp;rbrack；新工具栏用于直接访问特定的滤镜（裁切、变换、透视变换、仿制图章）
...
* 现在可以直接在视口中访问&amp;lbrack；UI&amp;rbrack；分辨率构件
* &amp;lbrack；UI&amp;rbrack；现在所有UI元素都是动态加载的
* &amp;lbrack；UI&amp;rbrack；快捷键 — 使用“2”可切换2D视图的可见性
...
* 资源面板&amp;lbrack；资源面板现在支持以下资源类型：材质、滤镜和图像
...
* 资源面板(&amp;L)&amp;L；按材料、筛选器或图像筛选当前文件夹
* 资源面板&amp;brack；组合多个滤镜以仅获取素材和图像(&amp;L)
...
* 图层面板&amp;brack；快捷键 — Shift + B可在图层上添加基础材质(&amp;L)
* 图层面板现在具有缩略图预览(&amp;L)（素材缩略图、滤镜图标或图像预览）
...
* &amp;lbrack；导出&amp;rbrack；将材质和环境光照发送到Adobe Substance 3D Painter
* &amp;lbrack；导出&amp;rbrack；将环境光发送到Adobe Substance 3D Stager
* &amp;lbrack；Rendering&amp;rbrack；现在支持新材质属性并在3D中渲染
* &amp;lbrack；渲染&amp;rbrack；添加光泽支持（光泽颜色、光泽不透明度和光泽粗糙度）
* &amp;lbrack；渲染&amp;rbrack；添加涂层支持（涂层颜色、涂层粗糙度、涂层标准、涂层Specular level和涂层IOR）
* &amp;lbrack；渲染&amp;rbrack；添加各向异性支持（各向异性级别和各向异性角度）
* &amp;lbrack；渲染&amp;rbrack；添加Specular edge color支持
* &amp;lbrack；渲染&amp;rbrack；在“通道设置”面板中激活这些新属性
* &amp;lbrack；渲染&amp;rbrack；引入新的实时引擎(2021) Beta版渲染器
* &amp;lbrack；渲染&amp;rbrack；在“查看器设置”面板中的两个渲染器版本之间切换
* &amp;lbrack；Rendering&amp;rbrack；实时引擎(2021)渲染器支持半透明、吸收和散射材料属性
* &amp;lbrack；Rendering&amp;rbrack；实时引擎(2021)渲染器引入了一种从环境光计算阴影的新方法
* &amp;lbrack；Rendering&amp;rbrack；实时引擎(2021)渲染器实时计算环境光的辐照度
* &amp;lbrack；着色器设置面板&amp;rbrack；新着色器设置面板可调整特定的材质着色器参数
* &amp;lbrack；着色器设置面板&amp;rbrack；新参数（正常比例、Height比例、Height级别、发射强度、IOR、涂层正常强度和涂层IOR）
* &amp;lbrack；着色器设置面板&amp;rbrack；实时引擎2021的特定参数（子表面散射、散射距离、红移和瑞利散射）
...
* &amp;lbrack；Engine&amp;rbrack；Substance 引擎更新
* &amp;lbrack；用于禁用特定功能的应用程序和rbrack；环境变量
* &amp;lbrack；Content&amp;rbrack；替换颜色 — 新的高级分段选项
* &amp;lbrack；Content&amp;rbrack；地板拼贴 — 可用的新图案和选项
...
* &amp;lbrack；Content&amp;rbrack；表面浮雕 — 使用Perlin分形噪点（高级噪声）修复拼贴问题
* &amp;lbrack；Content&amp;rbrack；墙式图案 — 16位Height输入
* &amp;lbrack；Content&amp;rbrack；材质图标渲染 — Specular反射问题
...
* &amp;lbrack；图像到材质&amp;rbrack；支持NVIDIA RTX 3000系列
* &amp;lbrack；图像到材质&amp;rbrack；用于控制几何细节的新参数
* &amp;lbrack；图像到材质&amp;rbrack；用于控制粗糙度的新参数
* &amp;lbrack；图像到材质&amp;rbrack；用于控制愉悦强度的新参数
* &amp;lbrack；缩略图&amp;rbrack；基于Substance DesignerPBR渲染器的新缩略图生成器
* &amp;lbrack；缩略图&amp;rbrack；更新基础材质和地图集以嵌入缩略图
* &amp;lbrack；缩略图&amp;rbrack；从.sbsar文件（如果存在）检索缩略图
* &amp;lbrack；缩览图&amp;rbrack；在“首选项”中更改缩览图品质
* &amp;lbrack；Engine&amp;rbrack；更新为Substance 引擎版本8
* &amp;lbrack；本地化&amp;rbrack；中文本地化
* &amp;lbrack；UI&amp;rbrack；实验性专色拾色器
* &amp;lbrack；Content&amp;rbrack；新环境映射 — Studio 06
...
* &amp;lbrack；Content&amp;rbrack；变换 — 添加在特定通道上停用变换的可能性
...
* &amp;lbrack；图层&amp;rbrack；防止在执行多个撤消/重做操作时崩溃
* &amp;lbrack；Layers&amp;rbrack；使用“图像到材质”（AI驱动）时崩溃，并返回日志：设备顺序无效
* &amp;lbrack；Filters&amp;rbrack；改进NVIDIA显卡检测以实现NVidia的特定功能
* 关闭应用程序时应用程序崩溃(&amp;L)
* &amp;lbrack；应用程序&amp;rbrack；修复MacOS上的VRAM数量检测
* &amp;lbrack；Export&amp;rbrack；某些导出预设有时缺失
* &amp;lbrack；Content&amp;rbrack；油画效果 — 使用高位移振幅固定Height范围
...
* 当图像到材质（AI驱动）内存不足时，出现&amp;lbrack；Layers&amp;rbrack；应用程序内错误消息

**已修复：**

* &amp;lbrack；图层&amp;rbrack；图像到材质（AI驱动）不适用于Specular/光泽度工作流程
* &amp;lbrack；Layers&amp;rbrack；在使用“图像到材质”（AI驱动）时视频内存不足时崩溃
* &amp;lbrack；Layers&amp;rbrack；磁盘缓存在打开栈栈时不用于显示
* Nvidia RTX 8000的&amp;lbrack；图层和rbrack；检测
* Layers&amp;rbrack；Layers&amp;rbrack；有时不可能将图层移到“飞溅”输入之外(&amp;L)
* 在栈栈中插入栈栈时，不使用&amp;lbrack；图层&amp;rbrack；磁盘缓存
* &amp;lbrack；Layers&amp;rbrack；虽然未使用某些通道用法，但会计算这些用法
* 导入图像时，有时会创建空白输出(&amp;l)
* &amp;lbrack；2D视图&amp;rbrack；切换至绘图模式下的另一图层活动块平移和缩放
* &amp;lbrack；Content&amp;rbrack；Snow — 正常映射上的8位问题
* &amp;lbrack；Content&amp;rbrack；路面图案 — 正常地图上的8位问题
* &amp;lbrack；Content&amp;rbrack；均衡器 — 正常映射上的8位问题
* &amp;lbrack；Content&amp;rbrack；砾石生成器 — 正常映射上的8位问题
* &amp;lbrack；Content&amp;rbrack；地板拼贴 — 手柄不透明度和Specular level
* &amp;lbrack；Content&amp;rbrack；混合器循环需要导出预设 — 反转法线图
* &amp;lbrack；Content&amp;rbrack；利用图像到材质（AI驱动）更正大型图像的问题
* &amp;lbrack；Application&amp;rbrack；在出现数据库错误时选择“备份并重新启动”时崩溃
* 快速单击同一资源时&amp;lbrack；Application&amp;rbrack；崩溃
* lbrack；Application&amp;rbrack； Rare在退出时崩溃(&amp;L)
* 将文件拖放到欢迎屏幕时&amp;lbrack；Application&amp;rbrack；崩溃
* 加载损坏的环境文件时应用程序崩溃(&amp;L)
* &amp;lbrack；Application&amp;rbrack；快速切换渲染的资源时发生罕见崩溃
* &amp;lbrack；应用程序在资源计算时退出时冻结(&amp;R)
* &amp;lbrack；Application&amp;rbrack；在macos上启动时极少崩溃
...
* Windows和Linux上提供的&amp;lbrack；Create&amp;rbrack；New Image to Material (AI powered)过滤器
* &amp;lbrack；Create&amp;rbrack；将位图重命名为Material to Image to Material (B2M)
* &amp;lbrack；图像导入&amp;rbrack；新材质创建模板弹出窗口
* &amp;lbrack；图像导入&amp;rbrack；新的“添加基础材质”选项
* &amp;lbrack；图像导入&amp;rbrack；能够在材质创建模板中拖放其他图像
* &amp;lbrack；图像导入&amp;rbrack；能够删除材质创建模板中的图像
* &amp;lbrack；图像导入&amp;rbrack；根据导入位图的文件名自动为其分配通道
* &amp;lbrack；图像导入&amp;rbrack；能够反转正常映射
* &amp;lbrack；2D视图&amp;rbrack；绘画模式简介
* &amp;lbrack；2D视图&amp;rbrack；绘画拼贴
* &amp;lbrack；2D视图&amp;rbrack；设置画笔颜色的灰度值
* &amp;lbrack；2D视图&amp;rbrack；绘画时平移和缩放
* &amp;lbrack；2D视图&amp;rbrack； X快捷键可反转画笔灰度值
* &amp;lbrack；2D视图&amp;rbrack；&amp;lbrack；和&amp;rbrack；用于更改画笔大小的快捷键
* &amp;lbrack；2D视图&amp;rbrack；Ctrl（或Cmd） +鼠标滚轮更改画笔大小
* &amp;lbrack；2D视图&amp;rbrack；现在可以在使用仿制修补时修改源位置
* 拖放；图层&amp;R；按住Shift并拖放可自动散点地图集(&amp;L)
* &amp;lbrack；图层&amp;rbrack； Alt +拖放操作将插入材料作为贴花
* &amp;lbrack；Layers&amp;rbrack；从Substance Designer中轻松地公开转换矩阵
...
* &amp;lbrack；UI&amp;rbrack；能够在图像小组件中的导入图像上绘画
...
* &amp;lbrack；性能&amp;rbrack；图像到材质（AI驱动）在RTX GPU上运行速度更快
* &amp;lbrack；Content&amp;rbrack；新网格：女性T恤、男性T恤、鞋
* &amp;lbrack；Content&amp;rbrack；新混合模式 — 每个通道混合
* &amp;lbrack；Content&amp;rbrack；使用2个新参数（Height位置和Height比例）进行不透明度混合Height校正
* &amp;lbrack；Content&amp;rbrack；在Height混合模式中添加Height调整
* &amp;lbrack；Content&amp;rbrack；使用“自定义蒙版混合”中的Height信息选项
* &amp;lbrack；Content&amp;rbrack；全新透视校正工具
* &amp;lbrack；Content&amp;rbrack；图案生成器 — 添加参数以反转图案
* &amp;lbrack；Content&amp;rbrack；图案生成器 — 添加一个新参数覆盖材质细节
* 标记；内容标记；新建贴花滤镜(&amp;R)
* &amp;lbrack；Content&amp;rbrack；新Moss筛选器
* &amp;lbrack；Content&amp;rbrack；新裂缝过滤器
* &amp;lbrack；Content&amp;rbrack；新建PBR 验证过滤器
* &amp;lbrack；Content&amp;rbrack；新建地板拼贴滤镜
...
* 加载导入的图像太多次(&amp;L)
* &amp;lbrack；Layers&amp;rbrack；在栈栈底部创建克隆修补程序时崩溃
* &amp;lbrack；Layers&amp;rbrack；在栈栈底部添加材质会导致其不稳定
...
* 在环境映射之间来回加载时应用程序崩溃(&amp;L)
* 安装某些图形输入板驱动程序时出现应用程序和Rbrack；性能问题(&amp;L)
* &amp;lbrack；应用程序&amp;rbrack； EXR 32位文件导入为黑色
* 加载和卸载资源时，Application&amp;rbrack；崩溃(&amp;L)
* 从浏览切换到创建时&amp;lbrack；Application&amp;rbrack；崩溃
...
* &amp;lbrack；UI&amp;rbrack；在2D视图中切换“素材输出”和“图层输入”时，保留选定的通道
* &amp;lbrack；图层&amp;rbrack；修复了在基础材质中添加自定义声道时崩溃的问题
* &amp;lbrack；图层&amp;rbrack；处理图层时崩溃
* &amp;lbrack；Layers&amp;rbrack；自定通道不与保存的材质一起显示
* &amp;lbrack；Application&amp;rbrack；修复了导入资源时发生罕见的崩溃问题
* &amp;lbrack；Application&amp;rbrack；退出时崩溃
...
* &amp;lbrack；导出&amp;rbrack；导出时崩溃
* &amp;lbrack；渲染&amp;rbrack；修复了基色为16位半浮点格式时的渲染
* &amp;lbrack；导入损坏的包时，Project&amp;rbrack；不崩溃
* &amp;lbrack；Project&amp;rbrack；从未打开创建时处理2019.1.4到2.x.x的迁移
* &amp;lbrack；Project&amp;rbrack；修复在两次导入同一项目时出现的崩溃问题
* &amp;lbrack；Project&amp;rbrack；修复导入项目时出现的崩溃问题
* &amp;lbrack；资源在早期版本中导入的自定义筛选器有效(&amp;R)
* &amp;lbrack；资源具有相同名称的材质不再相互擦除(&amp;R)
* 链接本地文件夹时资源崩溃(&amp;L)
* &amp;lbrack；Resources&amp;rbrack； Starter Materials用户创建的文件夹在重新启动后不再删除
* &amp;lbrack；Inspire&amp;rbrack；修复素材/收藏夹放置区域，并在使用未保存的素材时添加警告消息
...
* &amp;lbrack；Application&amp;rbrack；启动时打开创建，并更改实验室顺序
* &amp;lbrack；欢迎屏幕&amp;rbrack；新的欢迎横幅
* &amp;lbrack；Project&amp;rbrack；启动时打开上一个项目
* 打包；UI&amp;rbrack；新建组合框样式(&amp;L)
* &amp;lbrack；2D视图&amp;rbrack；在2D视图中聚焦的快捷键
* &amp;lbrack；Filters&amp;rbrack；在Substance图中添加了对alchemist：：parameterVisibility标记的支持
...
* &amp;lbrack；Project&amp;rbrack；两次导入同一项目时崩溃
* &amp;lbrack；Project&amp;rbrack；修复了导入和打开项目数次时崩溃的问题
* 加载未命名材质时应用程序崩溃(&amp;L)
* &amp;lbrack；Application&amp;rbrack；重新导入缺失的文件时识别它们
* &amp;lbrack；应用程序&amp;rbrack；修复关机时的随机崩溃
* &amp;lbrack；Application&amp;rbrack；修复了在Create中卸载材料时发生罕见崩溃的问题
* &amp;lbrack；Application&amp;rbrack；修复了使用UI控件时的随机崩溃问题
...
* &amp;lbrack；UI&amp;rbrack；单击材质时总是打开/关闭调整面板
* 打包；UI&amp;rbrack；修复隐藏图层颜色(&amp;L)
* &amp;lbrack；UI&amp;rbrack；欢迎屏幕按钮改进
* Lbrack；Layers&amp;rbrack；不必要的重新计算(&amp;L)
* 使用仿制修补时，Layers&amp;rbrack；崩溃(&amp;L)
* &amp;lbrack；Layers&amp;rbrack；选择图像导入图层不再触发计算
* 选择后，&amp;lbrack；图层重新计算&amp;rbrack；仿制修补和内容识别填充图层不再重新计算
* &amp;lbrack；通道设置&amp;rbrack；启用或禁用使用现在会触发渲染
* &amp;lbrack；资源&amp;rbrack；防止在批量单击库中的栈栈时出现冻结
* 重新添加以前添加的链接文件夹时&amp;lbrack；资源和rbrack；性能受到影响
* &amp;lbrack；资源&amp;rbrack；修复了尝试打开已删除的.sbsar文件时崩溃的问题
* &amp;lbrack；Performance&amp;rbrack；避免加载材料以访问其参数
* 仅在项目或创作的素材中使用时(&amp;L) Performance&amp;R；备份资源
* &amp;lbrack；Export&amp;rbrack；导出队列中的固定素材有时会被跳过或用错误的参数导出
* &amp;lbrack；2D视图&amp;rbrack；恢复平移和缩放
* &amp;lbrack；Content&amp;rbrack；镶板图案考虑环境遮蔽声道
* &amp;lbrack；Content&amp;rbrack；绘画 — 启用自定义蒙版时显示蒙版输入
* &amp;lbrack；Content&amp;rbrack； Stonewall图案 — 移除正常映射中可能的带状效果
* &amp;lbrack；Content&amp;rbrack；Height调制 — 校正2d视图中的双基色条目
...
* &amp;lbrack；Workflow&amp;rbrack；支持PBRSpecular光泽工作流程
* &amp;lbrack；Workflow&amp;rbrack；新建通道设置面板
* &amp;lbrack；创建项目时的工作流选择&amp;rbrack；工作流
* &amp;lbrack；通道设置&amp;rbrack；激活/停用特定通道计算
* &amp;lbrack；通道设置&amp;rbrack；显示当前素材中可用的自定义通道的列表
* &amp;lbrack；通道设置&amp;rbrack；需要时自动计算自定义通道
* &amp;lbrack；通道设置&amp;rbrack；强制/阻止自定义通道的计算
* &amp;lbrack；图层&amp;rbrack；Atlas Scatter和飞溅滤镜中素材输入占位符的新UI
* 滤镜的&amp;lbrack；图层&amp;rbrack；图像输入参数可通过下面的图层提供
...
* &amp;lbrack；Layers&amp;rbrack；删除图层时取消设置“图像输入”参数
* &amp;lbrack；图层&amp;rbrack；修复添加仿制修补图层时出现的崩溃问题
* &amp;lbrack；图层&amp;rbrack；修复在其他图层栈叠素材中混合图层栈叠素材时出现的某些崩溃问题
* 现在遵循导出的&amp;lbrack；Export&amp;rbrack；通道选择
* &amp;lbrack；资源在“资源”面板中导航时不崩溃(&amp;R)
* &amp;lbrack；资源&amp;rbrack；修复在导入损坏的Substance文件时崩溃的问题
* &amp;lbrack；资源&amp;rbrack；减少加载大型文件夹时崩溃的次数
...
* &amp;lbrack；应用程序&amp;rbrack；修复在退出时崩溃
* 导出材质时，应用程序&amp;rbrack；修复在退出时崩溃(&amp;L)
...
* &amp;lbrack；Performance&amp;rbrack；修复在材质之间快速切换时崩溃的问题
* &amp;lbrack；Export&amp;rbrack；在导出结束之前切换到其他材质不会再崩溃
* 右键单击素材时，&amp;lbrack；资源&amp;rbrack；上下文菜单显示在素材顶部
* &amp;lbrack；Layers&amp;rbrack；当图层栈栈为空时，“单击此处”链接有效
* 如果是在Alchemist中创建的素材，&amp;lbrack；预设&amp;rbrack；删除“调整”面板中的“存储”按钮
...
* &amp;lbrack；Engine&amp;rbrack；更新Substance引擎版本以避免在导入期间某些SBSAR文件崩溃。
* &amp;lbrack；Content&amp;rbrack；拼贴滤镜正在环境遮蔽通道上工作
* &amp;lbrack；Content&amp;rbrack；裁剪滤镜正在环境遮蔽通道上工作
* &amp;lbrack；Content&amp;rbrack；水滤镜修改获得Height图
* &amp;lbrack；Content&amp;rbrack；在不透明度混合模式下更正顶部素材的拼贴
* &amp;lbrack；顶部素材的Content&amp;rbrack；Height在不透明度混合模式下保留
* &amp;lbrack；Content&amp;rbrack；可在“穿孔”滤镜中添加自定义蒙版、自定义图案或比例图
* &amp;lbrack；Content&amp;rbrack；Height调制滤镜强制16位的Height和正常映射
* &amp;lbrack；Content&amp;rbrack；调整筛选器强制16位的Height和正常映射
...
* brack；混合&amp;brack；新不透明度混合模式(&amp;R)
* &amp;lbrack；引擎&amp;rbrack；新Substance 引擎版本

**已修复：**

* &amp;lbrack；Layers&amp;rbrack；修复在删除仍在计算的图层时崩溃的问题
* &amp;lbrack；图层&amp;rbrack；修复在删除底部图层时出现的崩溃问题
* &amp;lbrack；Layers&amp;rbrack；修复当材质名称包含特殊字符时崩溃的问题
* &amp;lbrack；Layers&amp;rbrack；停止计算每个使用小部件的滤镜
* &amp;lbrack；Layers&amp;rbrack；避免在使用“仿制修补”和“内容识别填充”滤镜时发生崩溃
* &amp;lbrack；图层&amp;rbrack；修复在飞溅输入插槽中拖放滤镜时崩溃的问题
* &amp;lbrack；资源&amp;rbrack；修复在Substance Alchemist中链接本地文件夹或导入资源时崩溃的问题
* &amp;lbrack；Collection&amp;rbrack；修复在材质之间快速切换时崩溃的问题
* &amp;lbrack；UI&amp;rbrack；修复当值为空或平铺时无效时崩溃的问题，位移视口上的滑块
* &amp;lbrack；Inspire&amp;rbrack；修复在访问“Inspire”选项卡时出现的崩溃问题
* &amp;lbrack；Inspire&amp;rbrack；修复当激发刚刚保存的图层栈叠素材时崩溃的问题
* &amp;lbrack；性能&amp;rbrack；重型Substance材质和滤镜（拼贴）计算速度更快
* &amp;lbrack；帮助&amp;rbrack；修复导出日志文件
* &amp;lbrack；Content&amp;rbrack；随机化滤镜适用于所有渠道
* &amp;lbrack；Content&amp;rbrack；多角度工作流程会考虑所有扫描
* brack；Content&amp;rbrack； AO混合正确混合(&amp;L)
* &amp;lbrack；Content&amp;rbrack；曲率混合正确混合
* &amp;lbrack；Content&amp;rbrack；颜色ID混合正确混合
* 打包；内容打包；自定蒙版混合正确混合(&amp;R)
* &amp;lbrack；Content&amp;rbrack；用于修改粗糙度的固定调整滤镜
...
* &amp;lbrack；Project&amp;rbrack；导出包含集合及其材料的.alch项目
...
* &amp;lbrack；Content&amp;rbrack；新材质内容识别填充滤镜
* &amp;lbrack；Content&amp;rbrack；变换滤镜有一个安全变换选项

**已修复：**

* 以前，所有与“创建”相关的错误现在都不再有效（新的UI和体系结构版本）
* 工具提示不会隐藏顶部栏(3D、2D、2D/3D)中的图标
* &amp;lbrack；Content&amp;rbrack；飞溅滤镜接受具有完整Height映射的Atlas
* &amp;lbrack；Content&amp;rbrack；转换筛选器适用于图像(scan1、scan2、...)
...
* &amp;lbrack；UX&amp;rbrack；在切换到其他实验室时保持左面板可见
...
* &amp;lbrack；Inspire&amp;rbrack；修复使用10种以上颜色时崩溃的问题
* &amp;lbrack；2D视图&amp;rbrack；在2D视图的通道列表中修复滚动条
* &amp;lbrack；Viewer&amp;rbrack；修复导入非2次幂环境映射时崩溃的问题
...
* &amp;lbrack；元数据&amp;rbrack；右键单击材质以查看其元数据
* &amp;lbrack；元数据&amp;rbrack；右键单击材质以查看其在磁盘上的位置
...
* &amp;lbrack；更新UI&amp;rbrack；“创建新材质”图标
* &amp;lbrack；帮助&amp;rbrack； URL更新为substance3d.com域
* 现在提供了&amp;lbrack；Mesh&amp;rbrack；布料网格
...
* &amp;lbrack；Content&amp;rbrack；现在可直接在基础材质过滤器中导入纹理
...
* &amp;lbrack；Engine&amp;rbrack；Substance 引擎更新以与最新Substance Designer版本兼容
* &amp;lbrack；License&amp;rbrack；更新首次安装的许可证文件夹
* &amp;lbrack；Layers&amp;rbrack；可随时重新加载图层栈栈，以更新自定义滤镜
...
* &amp;lbrack；元数据&amp;rbrack；在专用选项卡中查看和填充材质元数据
...
* &amp;lbrack；Content&amp;rbrack；新金属通用材料
* &amp;lbrack；Content&amp;rbrack；新铁锈材料
* &amp;lbrack；Content&amp;rbrack；新石材类材料
* &amp;lbrack；Content&amp;rbrack；浮雕过滤器更新
* 标记；内容标记；刺绣滤镜更新(&amp;R)
* &amp;lbrack；Content&amp;rbrack；绘画滤镜更新
* &amp;lbrack；Content&amp;rbrack； Delighter滤镜更新

**已修复：**

* &amp;lbrack；Content&amp;rbrack；水滤镜在Specular/光泽工作流程中工作
* 在激活弹出窗口中修复灰度单选按钮
* 接受包含昏迷字符的文件
* 修复弹出窗口上的小字体问题
...
* &amp;lbrack；Content&amp;rbrack；新建变换滤镜

**已修复：**

* &amp;lbrack；视口&amp;rbrack；球面网格，在X上平铺x2
* 加载您自己的环境时&amp;lbrack；Viewport&amp;rbrack；崩溃
* &amp;lbrack；视口&amp;rbrack；环境映射现在也使用曝光值
* &amp;lbrack；视口&amp;rbrack； F快捷键不会重置相机角度
* &amp;lbrack；导出&amp;rbrack； SBS导出适用于最新的Substance Designer2018.3.3
* &amp;lbrack；导出&amp;rbrack； SBSAR导出遵循与Substance Source材料相同的准则
...
* 分层；图层栈叠&amp;rbrack；图层重新排序(&amp;L)
* &amp;lbrack；图层栈栈&amp;rbrack；删除隐藏图层
* &amp;lbrack；图层栈栈&amp;rbrack；在您选择的位置直接导入材质
* &amp;lbrack；图层栈栈&amp;rbrack；材质输入作为新的滤镜参数类型
...
* &amp;lbrack；Export&amp;rbrack；导出前命名收藏夹或素材
* 按Ctrl+Shift+S组合键将&amp;lbrack；Data&amp;rbrack；另存为素材
* 按Ctrl+S保存材料(&amp;L)
* &amp;lbrack；Data&amp;rbrack；集合和材质在不同版本之间兼容
...
* “位图转换为材质”无法处理“Specular/粗糙度”工作流程
* 不建议在一个素材中使用多个喜悦
* 使用较旧的NVIDIA驱动程序（低于400.x）时Delighter崩溃
* Delighter阶段的快速可见性切换会影响性能
...
* 资源打包；资源打包；保存材质的新方式，以便在下一个版本中保持持久性(&amp;L)
* &amp;lbrack；帮助&amp;rbrack；通过帮助菜单访问联机文档
* &amp;lbrack；Performance&amp;rbrack；加快使用Substance Alchemist创建的复杂素材的颜色变化
* &amp;lbrack；Performance&amp;rbrack；减少切换实验室时的内存泄漏
* &amp;lbrack；Content&amp;rbrack；缩放检查器用于诊断素材的物理尺寸
* &amp;lbrack；Content&amp;rbrack；更新Italien Venice马赛克拼贴材料
* &amp;lbrack；Content&amp;rbrack；更新苔藓飞溅

**已修复：**

* 保存材质时不再使用默认名称
* 保存材质并重新打开Substance Alchemist后，滤镜参数丢失
* &amp;lbrack；Content&amp;rbrack；为AO和曲率混合从底逻辑和从顶逻辑修复
...
* &amp;lbrack；滤镜&amp;rbrack；导入自定义蒙版以管理两种材质之间的混合
* 标记；过滤器&amp;标记；控制苔藓缩放(&amp;R)
...
* 具有4种混合类型的材质混合（Height混合、样本混合、曲率混合、AO混合）
* 引入缓存机制以优化图层栈栈重新计算
* 如果显示在视区中，则在Inspire中自动选择素材
* “标准格式”集中在“材质设置”面板中
* 裁剪和平铺小组件控件（–90xB0，+90xB0，绘制正方形……） 清理
* 新建Snow过滤器
