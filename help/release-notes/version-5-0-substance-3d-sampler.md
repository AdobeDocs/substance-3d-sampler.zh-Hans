---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-sampler/release-notes/version-5-0-substance-3d-sampler.html"
breadcrumb-title: ''
...
<b>Substance 3D Sampler 5.0</b>引入了更简单的方法，可使用更高质量的扫描和渲染来生成素材数字孪生。

主要新增功能包括：

## 快速操作

一键启动Sampler的所有主要工作流程，并为您准备图层栈栈！
...
在实时和路径跟踪之间进行选择，以提高视觉一致性并支持新的材质属性。 直接从3D视图中保存您的工作快照。
...
借助HP Z Captis和Substance 3D Sampler，只需短短几分钟即可将真实材料变为数字材料。
...
* [用户引导] [内容]快速操作是预定义的工作流，可在图层栈栈中填充最常用的图层
* [入门]可以通过新的快速入门菜单、快速操作或自定义项目创建新项目
* [入门]可通过专用按钮直接从主页创建空项目
* [3D View]新的高级栅格化器和路径跟踪器在Substance生态系统中带来了新的渲染功能（如涂层、光泽、半透明、次表面散射等属性）和视觉一致性
* [3D视图]现在可以直接在3D视图中访问查看器设置
* [3D视图]可以将渲染快照保存在剪贴板或文件中
* [3D视图]显示网格以可视化场景原点
* [3D视图]启用地平面以捕捉阴影和反射
* [3D视图]控制地平面如何反光和不透明
* [3D 捕捉]将网格放置在地面上
* [应用程序]在应用程序启动时检查硬件兼容性
* [应用程序]崩溃报告窗口现在会在发生崩溃后立即打开
* [内容]打开示例项目以轻松开始
* [导出]以USD文件格式导出Adobe标准素材着色器
* [生成式AI]在“图像到纹理”工作流程中使用图像作为输入时，请勾选“不推断”标记
...
* [Content] Color ID Blend — 修复基色中的拾色问题
* [图层]在使用生成器时避免无用的计算
* [图层]调整生成器可能会导致触发太多计算
* [性能]改进GPU内存管理
