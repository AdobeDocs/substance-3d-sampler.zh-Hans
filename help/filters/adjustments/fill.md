...
“应用于……”下拉列表确定填充滤镜影响的通道。
**此列表中只显示当前在材料的声道设置中启用的声道。** 如果要填充的通道不可用：
...
切换切换开关以打开自定义映射。 选择要替换所选通道的图像，或直接在&#x200B;**2D视图**&#x200B;中绘画。
* 混合模式
复制、添加（线性减淡）、减去、正片叠底、添加子集、最大值（变亮）、最小值（变暗）、切换、分开、叠加、屏幕、柔光。
选择混合模式以将自定义输入与下面的图层混合。
* 不透明度
相对于现有信道信息调整新信道信息的不透明度。 换句话说，这控制用于应用新通道填充的蒙版的不透明度。

此模式适用于定义均匀的物理属性，如常量粗糙度或不透明度值。
...
在处理通过模板引入的高级材料行为时，此模式特别有用，因为模板中的某些属性由标量值而不是映射控制。
...
* 从头开始创建素材时初始化通道
* 覆盖现有通道值
* 设置统一的物理属性（例如固定粗糙度或金属量）
* 在重建细节之前中和通道，如“正常”
* 快速调整高级属性，例如模糊、translucency或涂层值

因为“填充”滤镜会自动适应选定的渠道，所以它可在所有材料类型中提供一致且可预测的工作流程。
