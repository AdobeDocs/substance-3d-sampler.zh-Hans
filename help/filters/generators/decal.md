...
breadcrumb-title: ''
description: 使用Substance 3D Sampler中的贴花生成器为素材表面创建贴花图案和叠加纹理。
...
* **拼贴模式**：\
  ...
  H代表“水平”，V代表“垂直”。
* **底部素材颜色匹配**： 0-1\
  调整贴花素材的颜色以匹配其下方图层的颜色值。
* **正常混合模式**：\
  调整贴花素材和下层图层之间的法线混合方式
* **正常不透明度混合**： 0-1\
  更改贴花素材法线的不透明度
* **贴花Height位置**： 0-1\
...
* **贴花变换**：\
  调整贴花的矩阵变换值。 一般来说，只使用&#x200B;**2D视图**&#x200B;中的手柄来调整贴花的变换更容易。
* **贴花** **偏移**： -1到1\
  调整贴花的偏移量。
...
可以通过在输入槽中选择素材，在&#x200B;**属性面板**&#x200B;中调整贴花输入素材的参数。
