...
利用&#x200B;**地图集创建器** **筛选器**，可将材料和图像转换为地图集。 然后，您可以使用&#x200B;**Atlas Scatter**&#x200B;和&#x200B;**Atlas Splitter**&#x200B;等其他滤镜在材料中使用贴图集元素。

以下图像显示&#x200B;**地图集创建者**&#x200B;处理丛林绿叶之前和之后的地图集。

![](../../assets/3d-2d-filters-cropped-0041-atlas-creator-in.jpg)

在上图中，贴图集图像已导入并转换为材料，但它仍然不是贴图集材料，因为不透明度贴图不考虑单个元素。

![](../../assets/3d-2d-filters-cropped-0040-atlas-creator-out.jpg)

运行&#x200B;**贴图集创建器**&#x200B;后，将生成不透明度映射，并将贴图集元素之间的区域填充到base color通道中。
...
* **添加不透明度**：图像/画笔
  ...
  导入要用作蒙版的文件或使用画笔直接绘画&#x200B;**2D 视图**&#x200B;中应不透明的区域。
...
**贴图集创建器**&#x200B;用于将贴图集图像转换为材料贴图集。

1. 将源图像导入到图层堆叠。
1. 如果系统提示您选择材料创建模板，请选择要材料的图像。 否则，如果图像在图层堆叠中，请在图像上方添加&#x200B;**图像到材料（AI支持）滤镜**。
1. 等待&#x200B;**图像到材质**&#x200B;筛选器将源图像转换为材质。 调整参数，直到您对结果满意为止。
1. 将&#x200B;**Atlas创建器筛选器**&#x200B;添加到图层堆叠顶部。
1. 调整&#x200B;**Atlas创建器**&#x200B;的参数，直到您对结果满意为止。

1. 将图像添加到图层堆叠。 如果系统提示您选择素材创建模板，请选择&#x200B;**用作位图**。
1. 选择图像图层后，在&#x200B;**属性面板**&#x200B;中，将&#x200B;**输出用法**&#x200B;更改为&#x200B;**基色**。
1. 将&#x200B;**Atlas创建程序**&#x200B;添加到图层堆叠顶部。
