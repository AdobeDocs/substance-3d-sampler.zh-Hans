...
breadcrumb-title: ''
description: 了解如何使用Substance 3D Sampler中的“导出”面板将材料导出为文件或将其直接发送给其他应用程序。
...
* **Substance 3D Painter**：导入可在为资源添加纹理时使用的材料和环境。
* **Substance 3D Stager**：导入环境光以更改场景的氛围。 仅适用于环境光，对材料禁用。

材料始终以SBSAR发送，环境则以EXR发送。

## 导出

单击&#x200B;**导出为……**&#x200B;以导出您当前正在处理的资源。 从左侧菜单中选择是修改常规设置还是材料设置。

### 常规设置

选择“常规”设置后，您可以更改材料的名称和存储位置。 您还可以切换是否为材料创建子文件夹。 在以可创建多个文件的图像格式导出时，此功能非常有用。

### 材质设置

选择材料设置后，可以更改各种参数以控制材料的导出方式：
...
| 解决方法 | 更改导出的分辨率。 根据所选格式<br>，此选项的显示方式不同 <ul> <li> **SBSAR/SBS**：为材料选择默认宽度和Height。 这些可以稍后更新。 <li> **图像格式**：选择&#x200B;**图层输出**，以便按照图层堆叠定义的大小导出每个映射，或选择&#x200B;**覆盖全部**，以便指定导出的宽度和Height。 |
| 材质模型 | 选择导出为Adobe Standard Material还是OpenPBR材料。 选择的选项应取决于在管道中使用哪些其他应用程序。 系统将根据材质模型提供不同的渠道。 |
