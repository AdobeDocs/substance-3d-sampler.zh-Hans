---
helpx_url: "https://helpx.adobe.com/substance-3d-sampler/getting-started/export/default-presets/v-ray-next.html"
breadcrumb-title: ''
description: 了解如何使用“V-Ray下一步”预设从Substance 3D Sampler导出材质，以便与V-Ray渲染兼容。
helpx_creative_field: ""
helpx_description: Sampler > Getting Started > Export > Default Presets > V-Ray Next
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: V-Ray下一个
user-guide-description: ''
user-guide-title: ''
source-git-commit: 55277f7a92e97bf530dd2a2edf4e16c88bb57793
workflow-type: tm+mt
source-wordcount: '42'
ht-degree: 4%

---


# V-Ray下一个

| 预设 | 兼容性 | 打包输出说明 |
| --- | --- | --- |
| V-Ray下一个 | <ul data-preserve-html="true"><li data-preserve-html="true">PBR金属/粗糙度</li></ul> | **扩散&#x200B;****粗糙度****金属度&#x200B;****正常********自模糊****不透明度** |
