...
导出材料时，“导出”窗口的“材料设置”面板具有以下选项：

* <b>格式</b>：为导出的资源选择文件格式。
  * <b>SBSAR</b>：导出您的材料，以便在支持Substance材料的任何应用程序中使用。
  * <b>SBS</b>：导出您的材料，以便可以在Substance 3D Designer中打开。
  * <b>EXR、JPEG、PNG、TARGA、TIFF</b>：将材料导出为图像文件集合。

>[!NOTE]
>
> 普通声道和Height声道的位深度将被强制设置为16位。 其他声道将以8/16位导出，具体取决于您的材料和资源使用的滤镜。 根据文件格式，可以更改位深度，因为某些文件格式不支持高位深度。
...
* <b>材料类型</b>(SBSAR、SBS)：选择导出的材料的行为是否像标准材料、贴花或贴图集。 此设置可以更改支持SBSAR和SBS文件的其他应用程序处理它的方式。
...
* <b>分辨率(</b>SBSAR， SBS<b>)</b>：为材料选择输出分辨率。
  * 默认情况下，分辨率基于Sampler的全局参数。 如果您选择其他分辨率，Sampler将使用此新分辨率重新计算您的所有材料。 这可能会影响素材的最终外观。
...
* **材质模型**（使用默认预设时为所有格式）：为导出的纹理选择一种着色器标准。
  * 更改材质模型将影响导出文件的文件名。 例如，OpenPBR使用“金属性”，而不是使用“金属”的ASM。
...
在<b>材料设置面板</b>的右侧，可以看到可导出的声道及其分辨率的列表（默认声道和自定义声道）。
