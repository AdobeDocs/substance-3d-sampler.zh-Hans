...
+++Captis涵盖哪些用例？
该解决方案涵盖跨行业的用例（汽车、服饰、产品设计、媒体和娱乐、体系结构……）。 Studio模式支持桌面拍摄（可重复、高效且简单），而资源管理器模式支持移动拍摄“随时随地灵活，适应各种情况”。

+++

+++Captis可以扫描和捕获哪些材料类型？
可以扫描和捕捉任何材料类型，但具有多个透明外套图层（Captis范围中不包括汽车绘画）的情况除外。 某些特定材料可能需要在Sampler中进行其他处理才能优化结果。 请注意，处理算法将随时间不断优化。

+++

+++对材料样本大小或形状有何限制？样本是否需要平坦？
Captis可以扫描多种大小或形状的材料样本。 样品在样品盘上被送去磁铁，使样品变平。 可通过多种模式使用Captis捕捉材料示例：

* 工作室模式：在工作室、工作室或工厂中，Captis将采用长达30厘米x 30厘米的样本，并配有背光以保持不透明度。 样本托盘的深度为1.8厘米。

* 资源管理器模式：您可以在现场、现场或特殊环境中使用资源管理器环，并灵活捕获大于30cm x 30cm的样本。 当前限制：请注意，资源管理器模式仍是早期版本，尚未优化（截至2024年7月29日版本）。
...
HP Z Captis设备由Adobe Substance 3D Sampler完全控制和运行：您可以从Substance 3D Sampler预览和启动捕获，捕获完成后，它将自动将PBR通道加载为图层并创建3d材料。 您可以使用Sampler中提供的所有工具和滤镜继续处理您的材质。

在Substance 3D Sampler中捕获到材料后，可将其导出到Substance 3D套件的任何应用程序(Substance 3D Designer、Painter、Stager)和任何支持Substance的第三方应用程序，包括3DS Max、Maya、Blender、Unreal Fonts、CLO、Browzwear、VRED、Rhino、Cinema4D等（在此处查看完整列表： <https://www.adobe.com/products/substance3d/plugins.html>）。
...
+++拼贴是否自动完成？
拼贴是使用当前拼贴筛选器在Sampler图层堆叠中执行的。

自动拼贴过滤器可用于自动拼贴具有定义的重复结构或小图案的材料，每个方向至少具有3个图案。 在文档](../../filters/tools/auto-tiling.md)的[专用部分中了解有关此筛选器的更多信息。

+++

+++扫描的材料可以导出为哪些格式？
HP Z Captis由Adobe Substance 3D Sampler本机运行。 HP Z Captis可捕捉64张原始图像（可从您的本地文件夹中检索）和PBR映射（可从捕捉的原始图像中进行处理并在Substance 3D Sampler中自动加载）。 Substance 3D Sampler将基于PBR通道创建3d材料，捕捉后这些通道会自动加载到Sampler图层堆叠。

从Adobe Substance 3D Sampler中，可以将数字材料导出为Substance 3D Sampler中可用的任何导出格式：Substance文件（.SBS和.SBSAR文件）或位图纹理（包括.PNG、.JPG、.TIFF..）（请参阅Sampler文档网页上的详细信息： [https://helpx.adobe.com/substance-3d-sampler/getting-started/export.html](../../getting-started/export/export.md)）。

+++

+++在拍摄期间，LDR和HDR有何区别？
在预览期间，您可以在LDR（低动态范围）和HDR(高动态范围)之间选择输出类型。\
即使选择LDR，系统也会捕获HDR地图并将其保存在您的设备上。\
建议您选择LDR，因为这将使项目的大小在Sampler以及将使用该sbsar 文件的任何第三方应用程序中更易于管理。
...
HP Z Captis由Adobe Substance 3D Sampler本机运行。 在Substance 3D Sampler中捕获材料样本并将其数字化后，您可以无缝导出数字材料：
...
例如，您可以添加192.168.55.1 captis-device，然后在<b>Sampler的设置>存储和缓存>材料捕获> captis地址</b>中，将IP替换为captis-device
...
+++我把圆锥体从底部移到资源管理器环上，无法再扫描。
建议在从底座或资源管理器环上拔下HP Z Captis之前，先将其关闭。
...
+++我想更改已捕获材料和测光图像的存储路径。
现在，您可以在“编辑”>“首选项”>“材料和缓存”>“材料捕捉”中编辑所捕捉存储和光度图像的保存位置。
