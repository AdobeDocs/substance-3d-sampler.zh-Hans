---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-sampler/pipeline-and-integrations/hp-z-captis-support/your-first-capture-step-by-step.html"
breadcrumb-title: ''
...
1. <b>浏览内容</b>：它将打开您的文件资源管理器以浏览HP Z Captis设备的本地存储。
...
如果您之前进行过捕捉，则选定的材料类别、输出和捕捉分辨率将与您之前的捕捉相同。
...
* 默认情况下，只存储材料的PBR通道（Base color、正常、Height和不透明度）。\
...
将此选项设置为要扫描的材料类型，以便根据特定的材料进行微调生成映射。\
选择的默认类别是“结构”。 这将有助于优化粗糙度渠道的效果。

如果您要扫描的内容包含多种类型的材料，请选择最大类别的类别。
...
自动裁剪将使用背光来定义材料的轮廓，并在其周围放置目标区域(ROI)。 它不适用于同时数字化多个材料样本或材料非常透明的情况。
...
若要进行对齐，请<b>将包含清晰锐利信息的内容（例如，一张包含印刷文本的纸）放在拍摄空间的中心</b>，关闭抽屉，然后单击对齐按钮。 完成此操作后，您可以确保一切到位，技术区域位于扫描空间两侧，材料位于中心，如有必要，可使用HP Z Captis设备提供的磁铁将其固定在位，然后您就可以开始扫描材料。
...
* <b>处理</b>：处理照片以生成PBR通道（Base color、正常、Height、不透明度）
...
在此步骤中，您可以查看扫描结果。 将显示所有创建的声道（在资源管理器模式下，由于资源管理器环没有背光，因此不会创建不透明度）。

您可以选择将素材发送到Sampler，将其添加到您的项目并开始处理。
您也可以直接开始新的捕捉，而无需将其添加到项目中。
在这两种情况下，您都可以在计算机上的等效文件夹中找到扫描地图： C:\Users\username\Documents\Adobe\Adobe Substance 3D Sampler\Captis\材料

## 材质版本

退出HP Z Captis窗口后，通道（base color、正常、Height、粗糙度和不透明度，如相关）将作为图层添加到“图层”面板中。
