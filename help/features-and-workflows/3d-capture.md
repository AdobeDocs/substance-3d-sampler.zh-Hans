...
breadcrumb-title: ''
description: 了解如何在Substance 3D Sampler中使用3D 捕捉，使用摄影测量技术从真实世界的对象创建材料。
...
Sampler使用摄影测量将图像变换为带有纹理的网格。 摄影测量学是一门从图像中进行测量的科学。 它用于从照片中提取信息，创建3D模型和纹理。 该过程包括从不同角度拍摄物体的多张照片，然后处理图像以提取图像中特征的形状和位置的信息。
...
[Linux配置](https://helpx.adobe.com/substance-3d/unlisted/documentation/sadoc/3d-capture-set-up-on-linux-255426606.html)

Mac

* 强烈推荐Apple芯片设备（M1或M2）
* 基于Intel和AMD GPU，至少支持4 Gb的VRAM和射线追踪
...
对于相机RAW格式，我们建议使用Lightroom将照片转换为.jpeg格式。
...
经过配准后的点云为稀疏点云，所有特征都被检测到，所有相机的位置也已被确定。
...
可单击左侧面板上的图像以在关联的相机上帧点云。

您可以单击相机以帧其上的点云。

## 重建

当将纹理投影到3D模型上时，重构步骤从匹配特征生成对象的3D模型。
...
后期处理可帮助您根据需要以及要如何使用它来调整和优化网格和纹理。

重建结果可以生成具有数百万个多边形和高达16K纹理的网格。 这通常不会针对渲染、实时体验或AR体验进行优化。

您需要对结果进行后期处理，以减少多边形的数量并且不会丢失细节。

后处理步骤会自动链接4个步骤：

* 抽取：通过定义所需的脸部数来减少多边形的数量
* 展开：自动定义精简网格的接缝、展开和打包UV
* 重新投影：将摄影测量网格的颜色纹理重新投影到精简网格上
* 烘焙：将摄影测量网格中的正常、Height和AO细节烘焙到精简网格上。 这将确保将在抽取过程中丢失的所有网格详细信息转移到纹理映射中。
...
将某个版本添加到项目后，将创建一个包含多个图层的图层堆叠。

第一层为重建结果。

第二层（如果进行了某些后处理）是使用“3D 捕捉”窗口中定义的值的网格后处理层。 如果要使用其他设置，仍可以在此步骤编辑参数。

第三个图层是用于缩放、平移和旋转3D对象的网格变换图层。

在此阶段，您可以添加用于在材料上应用的滤镜，以编辑3D对象上的纹理。

![](../assets/main-window-texturing.png)

## 导出

在导出窗口中，您可以定义网格格式和材料设置（导出材料时的相同设置）。
...
1. 相机：使用高分辨率相机和具有良好图像质量和清晰度的镜头。 避免使用带有鱼眼镜头或广角镜头的相机，因为这可能引起几何扭曲，从而影响最终结果。
1. 方向：拍摄图像时，尝试保持相机水平并垂直于地面。 以一定角度拍摄的图像很难准确提取特征，并且可能会导致结果失真。
...
**手机和DSLR相机之间关于摄影测量的推荐是什么？**

移动电话和单反相机相机都可以用于摄影测量，但它们各有优缺点。 在决定使用哪种相机类型时，需要考虑以下几点：

1. 分辨率：单反相机相机的分辨率通常比手机高得多，这样可得出更详细和更准确的结果。 然而，随着手机相机的不断发展，一些高端手机相机的分辨率和图像质量可与一些低端DSLR相机媲美。
1. 相机校准：摄影测量依赖于精确的相机校准，使用手机相机比使用单反相机相机通常更难以实现。 某些移动电话相机具有可以使用的内置校准参数，但它可能不如单反相机相机的正确校准准确。
1. 电池续航时间和存储空间：与单反相机相机相比，移动电话相机的电池续航时间更有限。 因此，您必须计划在工作时给手机充电或携带额外的电池。 此外，您需要确保手机有足够的存储容量来处理大型图像文件。
1. 成本：单反相机相机通常比移动电话贵，还需要额外的配件，如三脚架和外置闪光灯装置。
1. 便携性：手机比DSLR相机更便携，并且当您遇到要拍摄用于摄影测量的有趣对象或场景时，更有可能将手机随身携带。

总之，这实际上取决于您的具体需求和项目的特点。 对于分辨率较低的项目，使用手机可能就足够了。 但是，如果需要高精度、高分辨率，单反相机相机可能是一个较好的选择。 此外，如果您计划定期拍照或为长期项目拍照，投资单反相机相机从长远来看可能是一个更具成本效益的解决方案。

**如何校准相机以限制对象上的模糊？**

相机标定是摄影测量中的重要步骤，它有助于校正镜头扭曲和其他可能影响最终结果精度的误差。 下面是您可以采取的一些步骤来校准相机并限制对象上的模糊：

1. 使用三脚架：为了保持相机稳定并减少模糊，在拍摄用于摄影测量的图像时，使用三脚架非常重要。 这将确保相机位于每个拍摄的相同位置，并有助于最大限度地减少相机移动。
1. 使用快门遥控器：要进一步减少相机移动，您可以使用相机上的快门遥控器或自拍器功能拍摄图像。 这有助于最大限度地减少因按下快门按钮而导致的任何相机抖动。
1. 调整快门速度：要减少由相机移动导致的模糊，您应该使用较快的快门速度。 经验法则是，使用至少与镜头焦距倒数相同的快门速度。 例如，如果您使用的是50毫米镜头，则应使用至少1/50秒的快门速度。
1. 使用高ISO：在低光照条件下，您可能需要使用较高的ISO来保持较快的快门速度并减少模糊。 但是，请记住，高ISO也会增加图像中的噪声，这可能会影响最终结果的精度。
