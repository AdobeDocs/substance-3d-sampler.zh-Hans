...
基本想法是<b>向您的光线添加滤镜</b>和<b>您的镜头</b>，并将它们设置为<b>相互垂直</b>。 这意味着您需要通过旋转滤镜来微调滤镜方向。 一旦Specular被设置好，来自光线的反射就会变得不可见。 这非常特别，扭曲滤镜可以突然完全消除偏振光中的所有眩光。
...
<b>需要不断在你的设置中完成旋转和匹配的偏光器</b>。 您的镜头滤镜需要完全垂直于所有光线，唯一方法是查看相机显示并进行调整。 首先，我喜欢将一张纸贴在闪光灯上，然后调整镜头滤镜来遮挡闪光灯的反射。 这只能通过拍照或干烧闪光灯来实现。 这里有一点介绍，您可以使用标记在镜头滤镜上标记正确的方向，然后尝试不再触碰镜头和闪光滤镜。

调整视频光线上的偏振状态是不同的，但更容易。 在移动光源或调整Height时，您需要不断调整光源。 只需<b>旋转工作表，直到它在相机显示屏上看起来正确为止</b>。

<b>每个在反射中出现的光源都需要进行极化</b>，因此可能必须关闭窗口或关闭屏幕。

正确设置后，您应该能够捕捉对象，就好像它是完全遮罩一样，没有任何反射，甚至没有光照。 就像在只应用了基色纹理的情况下查看网格一样，它可以让您捕捉难反射的对象。
